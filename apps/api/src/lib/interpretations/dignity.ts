...
const MOOLATRIKONA: Record<
  number,
  { sign: number; startDeg: number; endDeg: number }
> = {
...
export function getFullDignity(
  ...
  if (exalt && sign === exalt.sign) {
    ...
    if (orb < 10) {
      ...
      return {
        dignity: "exalted",
        explanation: `${strength} exalted at ${exalt.degree}° in ${getSignName(sign)}`,
      };
    }
  }
  ...
  if (debil && sign === debil.sign) {
    ...
    if (orb < 10) {
      return {
        dignity: "debilitated",
        explanation: `debilitated at ${debil.degree}° in ${getSignName(sign)}`,
      };
    }
  }
  ...
  if (mt && sign === mt.sign && degree >= mt.startDeg && degree < mt.endDeg) {
    return {
      ...
      explanation: `in Moolatrikona sign ${getSignName(sign)} — exceptionally strong`,
    };
  }
  ...
  if (planetId in SIGN_LORDS && SIGN_LORDS[sign] === planetId) {
    return {
      dignity: "own",
      explanation: `in its own sign ${getSignName(sign)}`,
    };
  }
  ...
  if (planetId >= 0 && planetId <= 6 && sign >= 0 && sign <= 11) {
    ...
    if (friendship === "friend") {
      return {
        dignity: "friendly",
        explanation: `in ${getSignName(sign)} — ruled by friend ${getPlanetName(signLord)}`,
      };
    }
    if (friendship === "enemy") {
      return {
        dignity: "enemy",
        explanation: `in ${getSignName(sign)} — ruled by enemy ${getPlanetName(signLord)}`,
      };
    }
    if (friendship === "neutral") {
      ...
      return {
        dignity: "neutral",
        explanation: `in ${getSignName(sign)} — ruled by neutral ${getPlanetName(signLord)}`,
      };
    }
  }
...
function getPanchamahapurushaYoga(
  planetId: number,
): PanchamahapurushaYoga | null {
  const yogas: Record<number, PanchamahapurushaYoga> = {
    2: {
      name: "Bhadra Yoga",
      planet: "Mercury",
      condition: "in own/exalted sign in kendra",
      description:
        "Bhadra Yoga confers intelligence, eloquence, wealth, and social grace. Native is learned, witty, and successful in business.",
    },
    3: {
      name: "Malavya Yoga",
      planet: "Venus",
      condition: "in own/exalted sign in kendra",
      description:
        "Malavya Yoga brings beauty, artistic talent, luxury, wealth, and a comfortable life. Native is charming and fortunate.",
    },
    4: {
      name: "Ruchaka Yoga",
      planet: "Mars",
      condition: "in own/exalted sign in kendra",
      description:
        "Ruchaka Yoga gives courage, strength, leadership, and commanding presence. Native excels in competitive fields.",
    },
    5: {
      name: "Hamsa Yoga",
      planet: "Jupiter",
      condition: "in own/exalted sign in kendra",
      description:
        "Hamsa Yoga confers wisdom, spirituality, wealth, and high moral character. Native is respected and fortunate.",
    },
    6: {
      name: "Shasha Yoga",
      planet: "Saturn",
      condition: "in own/exalted sign in kendra",
      description:
        "Shasha Yoga gives authority, discipline, longevity, and rise from humble beginnings to power. Native is a natural administrator.",
    },
  };
...
export function getRetrogradeDignity(
